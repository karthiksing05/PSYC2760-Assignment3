...
def generate_familiarization(words, duration_seconds=120, output_file="familiarization.wav"):
...
def generate_test_stimulus(word, output_file):
    text = " ".join([word] * 20)
...
def main():
    ...
    generate_test_stimulus(old_word, "old.wav")

    new_word = "".join(random.sample(SYLLABLES, 3))
    while new_word in WORDS:
        new_word = "".join(random.sample(SYLLABLES, 3))
    generate_test_stimulus(new_word, "new.wav")
